...
!#######################################################################
  ...
  subroutine get_subspaces(kpq,kpqf,kpqd,ndim,ndimf,ndimd,nout,noutf,&
       ndims,ndimsf)
...
!#######################################################################
  ...
  subroutine initial_space_diag(time,kpq,ndim,ndims,noffd)
    ...
    if (lmatvec) then
       ! Block Davidson diagonalisation with the direct calculation
       ! of matrix-vector products
       call initial_space_diag_direct(time,kpq,ndim,ndims,noffd)
    else
       ! Block Davidson or relaxation diagonalisation with the
       ! precalculation and storage of the Hamiltonian matrix
       call initial_space_diag_precalc(time,kpq,ndim,ndims,noffd)
    endif
...
!#######################################################################
  ...
  subroutine initial_space_diag_direct(time,kpq,ndim,ndims,noffd)
...
!-----------------------------------------------------------------------      
    if (method.eq.2) then
       msg='Calculating the initial space ADC(2)-s matrix-vector &
            multiplication'
    else if (method.eq.3) then
       msg='Calculating the initial space ADC(2)-x matrix-vector &
            multiplication'
    endif
...
!-----------------------------------------------------------------------
! Davidson diagonalisation with matrix-vector products calculated
! on-the-fly
!-----------------------------------------------------------------------
  ...
  end subroutine initial_space_diag_direct
      
!#######################################################################

  subroutine initial_space_diag_precalc(time,kpq,ndim,ndims,noffd)
...
!-----------------------------------------------------------------------
    if (method.eq.2) then
       msg='Calculating the initial space ADC(2)-s Hamiltonian matrix'
    else if (method.eq.3) then
       msg='Calculating the initial space ADC(2)-x Hamiltonian matrix'
    endif
    write(ilog,'(/,a)') trim(msg)
...
!-----------------------------------------------------------------------
  ...
  end subroutine initial_space_diag_precalc
...
!-----------------------------------------------------------------------
    if (lmatvec) then
       ! Block Davidson diagonalisation with the direct calculation
       ! of matrix-vector products
       call davidson_final_space_diag_direct(ndim,ndimf,ndimsf,kpq,&
            kpqf,noffdf)
    else
       ! Block Davidson or relaxation diagonalisation with the
       ! precalculation and storage of the Hamiltonian matrix
       call davidson_final_space_diag_precalc(ndim,ndimf,ndimsf,kpq,&
            kpqf,noffdf)
    endif
...
!#######################################################################
  ...
  subroutine davidson_final_space_diag_direct(ndim,ndimf,ndimsf,kpq,&
            kpqf,noffdf)
...
!-----------------------------------------------------------------------
  ...
  end subroutine davidson_final_space_diag_direct
    
!#######################################################################

  subroutine davidson_final_space_diag_precalc(ndim,ndimf,ndimsf,kpq,&
       kpqf,noffdf)
...
!-----------------------------------------------------------------------
  ...
  end subroutine davidson_final_space_diag_precalc
...
!-----------------------------------------------------------------------
    ...
    write(ilog,'(2x,a)') 'Calculating the initial space dipole moments'
    write(ilog,'(70a)') ('-',i=1,70)
...
!-----------------------------------------------------------------------
    ...
    call get_adc2_dipole_improved_omp(ndim,ndim,kpq,kpq,nbuf,nel,&
         filename)
...
! -----------------------------------------------------------------------
    ...
    call get_adc2_dipole_improved_omp(ndimf,ndimf,kpqf,kpqf,nbuf,nel,&
         filename)
...
!#######################################################################
  ...
  subroutine contract_dipole_state(filename,dim1,dim2,vec,tvec,nbuf,&
       nel,cntrdir)
...
!#######################################################################
  ...
  subroutine initial_space_tdm(ener,rvec,ndim,mtm,tmvec,osc_str,kpq)
