!
!  Evaluation of 1-electron matrix elements of complex absorbing potentials
!  using numerical integration.
!
!  This module must not be called from a parallel region.
!  The interface design for the cap_ parameters is a bit messy, but should do.
!
  module basis_cap
    use accuracy
    use timer
    ...
    !
    !  Calculate effective multiplicative CAP, and (if possible) compare it to the reference CAP
    !
    ...
    !
    !  CAPs are evaluated using numerical integration on a grid; this seems easier than
    !  trying to figure out an analytical solution using RI.
    !
