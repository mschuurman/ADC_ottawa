...
!!! SCRIVE I PEZZI HAMILTONIANI SU FILE ( IL PEZZO I , NEL FILE [100 + I] ) !!!
     call  save_hampieces_adc2( DIMEN , NSYMA_PROP , SYM_MAP ,DIM_PROP , &
          TOTDIM_PROP, NBUF_SYM)
!!! SCRIVE I PEZZI HAMILTONIANI SU FILE ( IL PEZZO I , NEL FILE [100 + I] ) !!!

     call sym_toprop_calc_dip2( DIPOLESYM , ELECTRIC_FIELD , NSYMA_PROP , &
          SYM_MAP , NBLOCKS_DIP )
...
!-----------------------------------------------------------------------
           ...
           call get_fspace_adc2_direct(ndimf,kpqf(:,:),arrf,enerf)
...
!-----------------------------------------------------------------------
           ...
           write(101,*) ndimf , travec !!! I WRITE IN 101 THE D psi_in STATE
           ...
           write(102,*) ndimf , travec_norm  !!! I WRITE IN 102 THE D psi_in STATE NORMALIZED
           ...
           write(103,*) MAP   !!! I WRITE IN 103 THE VECTOR CONTAINING THE MOST BIG VALUES OF THE D psi_in STATE
...
!-----------------------------------------------------------------------
! Davidson diagonalisation in the initial space
!-----------------------------------------------------------------------
        ELSE IF  (chrun2.eq.'davi') THEN
           ...
           ! Check to see if Hamiltonian files exist
...
! (lanczos for the final states and davidson for the initial state)
!-----------------------------------------------------------------------
     ELSE IF (chrun .eq. 'save') THEN
        write(*,*) 'I WILL PERFORM LANCZOS ADC2 DIAGONALIZATION IN THE FINAL SPACE'

! Davidson in the initial space first
        inquire(file='hmlt.diai',exist=prs)
        inquire(file='hmlt.offi',exist=prs1)
        inquire(file='fort.111',exist=prs2)
        if(prs .and. prs1 .and. prs2) then
...
!-----------------------------------------------------------------------
        ...
        ! vec_init s.t. indx(i) is the index of the element of
        ! init_vec that has the ith greatest (in magnitude) value ???
        call dsortindxa1("D",ndim,coeff(:),indx(:))
     
        ! Output the initial state number and energy as well as the
        ! dominant coefficients for this state
        write(6,*) statenumber,e_init,e_init*27.211396,(coeff(indx(j)),indx(j),j=1,5)
        write(105,*) e_init
        ...
        ! Allocate the array travec, which will hold the vector
        ! D|Psi_In> in the IS basis, where |Psi_In> is the initial
        ! state 
        allocate(travec(ndimf))

        ! Calculate the scalar product of the dipole matrix with the
        ! initial state vector in the IS basis,
        ...
        ! Output the array travec
        write(101,*) ndimf , travec

        ! Determine the dominant coefficients in the expansion of
        ! D|Psi_In> in the IS basis, i.e., the elements
        ! <Psi_m|D|Psi_In>, where |Psi_m> are the ISs
        allocate(coeff_tra(ndimf))
        allocate(indx_tra(ndimf))
        coeff_tra(:)=travec(:)**2
        call dsortindxa1("D",ndimf,coeff_tra(:),indx_tra(:))
        ...
        ! ??? MAP holds the indices of the configurations that will
        ! form the space spanned by the starting vectors ???
        !
        ! We take all 1h1p ISs and 100 2h2p ISs
        allocate(MAP(kpqf(1,0)+100))
        ...
        ! ??? First 100 2h2p ISs with the greatest TDMs with the
        ! initial state ???
        ! N.B. indx_tra contains the indices of the ISs in order of
        ! increasing absolute value of their TDMs with the initial
        ! state
        count = 0
        DO i = kpqf(1,0) + 1 , kpqf(1,0) + 100
           count = count + 1
        ...
        ! Output the MAP array
        write(103,*) kpqf(1,0) + 100 ! Dimension of MAP
        write(103,*) MAP
        
        ! Write the final space ADC(2) Hamiltonian matrix to disk
        ...
        IF ( GO .EQ. 'OK' ) THEN
           ...
           ! ??? Fill in the starting vectors for the block Lanczos
...
!-----------------------------------------------------------------------
     ...
     END IF ! END IF ON THE CHRUN CONDITION
     
  END IF ! END IF ON THE WHAT CONDITION
