...
77    close(nfl)
    ...
    nvecout=count
    write(ilog,*) nvecin,' Vectors requested - ', count, 'found on disc in the file ', fname
...
77    close(nfl)
    ...
    nvecout=count
    write(ilog,*) nvecin,' Energies requested - ', count, 'found on disc in the file ', fname
...
!!$----------------------------------------------------------------------
    subroutine read_vec(ndim,name,vec)
      ...
      if (ndim .ne. ndim1) then
         write(ilog,*) 'The vector size in ', name,' is wrong', ndim1
         stop
      end if
