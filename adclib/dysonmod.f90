...
!           and so must be multiplied by a factor of sqrt(two) if total
...
!#######################################################################
  ...
  module dysonmod
    ...
    real(d), parameter :: invsqrt2=1.0d0/dsqrt(2.0d0)
...
!-----------------------------------------------------------------------
      ...
      do n=count+1,count+kpq(5,0)
         ...
         do i=1,nocc
            ...
            ftmp=vpqrs(a,i,b,k)-vpqrs(b,i,a,k)
            smat(i,j)=smat(i,j)&
                 +delta*ftmp*(3.0d0/dsqrt(12.0d0))*vec_init(n)
         enddo
      enddo
      ...
      do n=1,ndims
         ...
         do m=count+1,count+kpqf(5,0)
            if (j.eq.kpqf(4,m).and.b.eq.kpqf(6,m)) then
               i=kpqf(3,m)
               a=kpqf(5,m)
               pmat(a,i)=pmat(a,i)-(6.0d0/dsqrt(24.0d0))*vec_final(m)*vec_init(n)
            endif
         enddo
...
!-----------------------------------------------------------------------
! CHECK: assume that the prefactor should be -1/2 instead of -2
!-----------------------------------------------------------------------
      pmat=pmat/4.0d0
...
!-----------------------------------------------------------------------
      ...
      do m=countf+1,countf+kpqf(3,0)
         do n=count+1,count+kpq(2,0)
            ...
            qmat(a,b)=qmat(a,b)+dsqrt(2.0d0)*vec_final(m)*vec_init(n)
         enddo
      enddo
...
!-----------------------------------------------------------------------
      ...
      do m=countf+1,countf+kpqf(5,0)
         do n=count+1,count+kpq(4,0)
            ...
            if (.not.lcontrib) cycle
            qmat(a,b)=qmat(a,b)+ &
                 (1.0d0/dsqrt(6.0d0))*vec_final(m)*vec_init(n)
         enddo
      enddo
...
!-----------------------------------------------------------------------
      ...
      do m=countf+1,countf+kpqf(5,0)
         do n=count+1,count+kpq(5,0)
            ...
            if (.not.lcontrib) cycle
            qmat(a,b)=qmat(a,b)+ &
                 ((1.0d0/6.0d0)+(4.0d0/dsqrt(24.0d0)))*vec_final(m)*vec_init(n)
         enddo
      enddo
