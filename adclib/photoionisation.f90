...
!!$ with the largest fosc is found and its number is saved. Inefficient procedure.
    ...
    do i= 1,norder-1
       ...
       write(ilog,*) 'The region ',i,' lies between ',ene(nbound),' and ', ene(nbound+ndelta)
       write(ilog,*) 'The dominant state in region', i,' is ',numfosc,' and carries',&
    ...
    write(ilog,*) 'The region ',i,' lies between ',ene(nbound),' and ', ene(ndim)
    write(ilog,*) 'The dominant state in region', i,' is ',numfosc,' and carries',&
         maxfosc/fsum,' osc.strength','(',maxfosc,')' 
    indx(i)=numfosc
...
102 FORMAT(5(F8.6,"(",I4,")",1x))
    ...
    do i= 1,norder
       ...
       write(ilog,*) 'Region ',i,' Conf.: ',(indx(j),j=1,5)
       write(ilog,102) (coeff(indx(j)),indx(j),j=1,5)
