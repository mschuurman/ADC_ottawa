...
!  MPFUN-2015: Transcendental function module (module MPFUND).
...
!    This module contains subroutines for transcendental constants and functions,
...
!    "Special functions", such as Gamma, BesselJ, are defined in module MPFUNE.

!  NOTE FOR EXTRA-HIGH PRECISION USAGE:
!    The requisite FFT and transcendental constant data are included in module
!    MPFUNA to support all operations in this package, up to approximately 19,500
!    digits.  If higher precision is required, you must call MPINIFFT (in module
!    MPFUNB) to initialize FFT data.  If you are also using the transcendental
!    functions in module MPFUND, then you must call MPINITRAN (in module MPFUND)
!    to initialize data for the transcendental functions.  Alternatively, if you
!    are using the high-level interface in module MPFUNF, calling mpinit will do
!    both tasks.  Thereafter, all functions may be used, entirely thread-safe.
!    See documentation for details.
 
!  FORMAT FOR A MULTIPRECISION DATA ELEMENT (64-bit double precision words):
!    Word 0: Total space allocated for array, in words.
!    Word 1: Working precision level (in words) associated with this data.
!    Word 2: Number of mantissa words N; sign of word 2 is sign of value.
!    Word 3: Exponent of value, base 2^48.
!    Word 4 through word N+3: mantissa (integers between 0 and 2^48 - 1).
!    Word N+4 and N+5: scratch; set to zero in output arrays by all
!       subroutines except mproun.
!    Note that N mantissa words require at least N+6 total array space.

!    Example: (12., 6., -2., 0., 3., 70368744177664., 0., 0., 0., 0., 0., 0.) 
!    represents -3.25 (since 70368744177664 = 0.25 * 2^48), and the working 
!    precision associated with this data is 6 (as specified in word 1).

!    A multiprecision complex (MPC) datum is a contiguous pair of multiprecision
!    real data.  The imaginary part is presumed to start at Word M, where M is
!    is the value of Word 0 of the real part.  The total array space for a MPC
!    datum is twice the value of Word 0, i.e., 2*M. 

!  CODE CONVENTIONS:
!    1. Subroutines in this module only call subroutines or reference data in this
!       module or in module MPFUNA, MPFUNB or MPFUNC.
!    2. All variables are strongly typed, i.e. implicit none is used throughout
!       and the space requirements for input and output variables are enforced. 
!    3. There are no global variables in this module, except for parameters
!       that can be changed only in source code.  Also, no initialization is
!       required. These features ensure that the entire module is thread-safe.
!    4. All working arrays needed by a subroutine in this module are allocated
!       in the declaration section of that subroutine.
!    5. All subroutines produce output MP arrays in the standard format above.
!    6. Each subroutine checks the first few words of input and output MP arrays,
!       to ensure that the arrays have been initialized and have adequate space.
!       In particular, these conditions must be met:
!       a.  mpnw (input working precision) must be >= 4.
!       b.  Word 0 of input MP arrays must be >= abs(a(2)) + 4.
!       c.  Word 0 of output MP arrays must be >= mpnw + 6.
!       If these conditions are not met, an error message is produced and 
!       execution is terminated (i.e., mpabrt is called).
!    7. Word 1 (i.e., the second word) of input MP variables is used to transmit
!       the working precision level from the language interface module (MPFUNF),
!       but is ignored in lower-level subroutines -- the argument mpnw takes 
!       precedence. Word 1 of output MP variable(s) is always set to the current 
!       working precision.
!    8. All error conditions produce an error message in the subroutine where
!       the error is detected, after which mpabrt is called with the error code.
!       Subroutine mpabrt (in module mpfunb) normally stops execution, but it
!       can be modifed by the user if desired to produce a system-dependent 
!       traceback.  Error messages are output on Fortran unit mpldb (see MPFUNA).
...
if (mpnw < 4 .or. a(0) < mpnw + 4 .or. b(0) < abs (a(2)) + 4 .or. &
  c(0) < mpnw + 6) then
  write (mpldb, 1)
1 format ('*** MPAGM: uninitialized or inadequately sized arrays')
  call mpabrt (99)
endif
...
write (mpldb, 2)
2 format ('*** MPAGM: Iteration limit exceeded.')
call mpabrt (5)
...
!   MPANGX instead.  By default, MPNWX = 30 (approx. 400 digits).
...
!   The formula is:  1/2 * Log[r] + I * Theta, where r = Sqrt[a1^2 + a2^2],
...
!   returns the two MPR results in X and Y, respectively.

!   If the precision level MPNW exceeds MPNWX, this subroutine calls
!   MPCSSHX instead.  By default, MPNWX = 100 (approx. 1430 digits).

implicit none
integer mpnw, mpnwx, mpnw1
parameter (mpnwx = 100)
...
!   If the precision level mpnw exceeds mpnwx, call mpcsshx.

if (mpnw > mpnwx) then
  call mpcsshx (a, x, y, mpnw)
  goto 100
endif
...
subroutine mpcsshx (a, x, y, mpnw)

!   This computes the hyperbolic cosine and sine of the MPR number A and
!   returns the two MPR results in X and Y, respectively.
!   For modest precision levels, call mpcsshr.
...
if (mpnw < 4 .or. a(0) < mpnw + 4 .or. a(0) < abs (a(2)) + 4 .or. &
  x(0) < mpnw + 6 .or. y(0) < mpnw + 6) then
  write (mpldb, 1)
1 format ('*** MPCSSHX: uninitialized or inadequately sized arrays')
  call mpabrt (99)
endif
...
return
end subroutine mpcsshx

subroutine mpcssnr (a, x, y, mpnw)

!   This computes the cosine and sine of the MPR number A and returns the two MPR
!   results in X and Y, respectively.  Pi must be precomputed to at least MPNW
!   words precision and the stored in the array in module MPMODA.
...
!   If the precision level MPNW exceeds MPNWX, this subroutine calls
!   MPCSSNX instead.  By default, mpnwx = 300 (approx. 4300 digits).

implicit none
integer i, ia, itrmx, i1, j, k, ka, mpnw, mpnwx, mpnw1, na, ndeg, nq, n1, n2
parameter (itrmx = 1000000, mpnwx = 300, nq = 2)
...
!   Compute of sin the reduced argument s1 using Taylor series.
...
!   Compute Cos (s1) = Sqrt [1 - Sin^2 (s)].

call mpeq (s0, s1, mpnw1)
call mpmul (s0, s0, s2, mpnw1) 
call mpsub (f, s2, s3, mpnw1) 
call mpsqrt (s3, s0, mpnw1)

!   Apply the double-angle formulas NQ times.

do j = 1, nq
  call mpmul (s0, s0, s4, mpnw1)
  call mpmul (s1, s1, s5, mpnw1)
  call mpsub (s4, s5, s2, mpnw1)
  call mpmuld (s0, 2.d0, s4, mpnw1)
  call mpmul (s4, s1, s3, mpnw1)
  call mpeq (s2, s0, mpnw1)
  call mpeq (s3, s1, mpnw1)
enddo
...
!   This computes the cosine and sine of the MPR number A and returns the two MPR
!   results in X and Y, respectively.  Pi and Log(2) must be precomputed to at
...
!   This computes the exponential function of the MPR number A and returns the
!   MPR result in B.  Log(2) must be precomputed to at least MPNW words
!   precision and the stored in the array in module MPMODA.
...
!   If the precision level MPNW exceeds MPNWX words, this subroutine calls 
!   MPEXPX instead.  By default, MPNWX = 100 (approx. 1430 digits).

implicit none
integer i, ia, itrmx, j, mpnw, mpnwx, mpnw1, na, nq, nz, n1, n2
double precision t1, t2
parameter (itrmx = 1000000, mpnwx = 100, nq = 12)
double precision a(0:mpnw+5), b(0:mpnw+5), f(0:8), s0(0:mpnw+6), s1(0:mpnw+6), &
  s2(0:mpnw+6), s3(0:mpnw+6), s4(0:mpnw+6)
...
do j = 1, itrmx
  t2 = dble (j)
  call mpmul (s2, s1, s0, mpnw1) 
  call mpdivd (s0, t2, s2, mpnw1) 
...
!   If the precision level MPNW exceeds MPNWX words, this subroutine calls
!   MPLOGX instead.  By default, MPNWX = 15 (approx. 210 digits).
...
parameter (alt = 0.693147180559945309d0, cl2 = 1.4426950408889633d0, &
  rtol = 0.5d0**7, itrmax = 1000000, nit = 3, mprxx = 1d-14, mpnwx = 15)
