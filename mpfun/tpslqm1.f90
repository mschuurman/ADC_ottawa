...
use mpmodule
implicit none
double precision d1, d2, drand, rb, rm, second, tm, tm0, tm1
integer i, idb, iq, itr, j, k, kps, kcs, kq, kr, ks, kss, n, ndp, nep
parameter (idb = 2, itr = 1, kq = 0, kr = 5, ks = 5, &
  n = kr * ks + 1, ndp = 180, nep = -170, rb = 100.d0)
integer lnm(n)
character*1 chr1(1050)
...
4   format ('Recovered relation: 0 =')
    ...
    do i = 1, n
      if (r(i) .ne. 0.d0) then
        call mpfform (r(i), 40, 1, chr1)
        
        do j = 1, 40
          if (chr1(j) /= ' ') then
             if (chr1(j) /= '-' .and. j >= 2) chr1(j-1) = '+'
             goto 110
          endif
        enddo

110     continue
        write (6, '(40a1,'' * '',a)') (chr1(j), j = 1, 40), nam(i)(1:lnm(i))
      endif
    enddo
...
tm0 = second ()
call itermp (idb, it, n, nsq, ip, ir, is, dx, eps, b, h, s, syq, t, y, &
  imq, izm)
tm1 = second ()
