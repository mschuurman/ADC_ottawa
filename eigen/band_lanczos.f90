...
!#######################################################################
    ...
    subroutine run_band_lanczos
      ...
      write(ilog,'(70a,/)') ('*',i=1,70)
      write(ilog,'(2x,a,x,i4,2x,a,x,i5,x,a,/)') 'A maximum of',&
...
!-----------------------------------------------------------------------
      ...
      if (j.ne.1.and.floor(frac)-frac.eq.0) then
         if (int(frac).eq.1) then
            write(ilog,'(70a)') ('-',i=1,70)
            write(ilog,'(2x,i4,2x,a)') int(frac),'Lanczos block  generated'
         else
            write(ilog,'(70a)') ('-',i=1,70)
            write(ilog,'(2x,i4,2x,a)') int(frac),'Lanczos blocks generated'
         endif
      endif
...
!-----------------------------------------------------------------------
      ...
      write(ilog,'(/,2x,a,/)') 'End of the band-Lanczos routine'
...
!#######################################################################
    ...
    subroutine deflate_kryvec(j,cblckdim,indxdfl,kryvec,matdim,&
      ...
      write(ilog,'(/,7x,a,/)') 'Deflating the subspace'
